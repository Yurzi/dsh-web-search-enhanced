...
function render(extra: Partial<Parameters<typeof SearchSelectorControl>[0]> = {}) {
  ...
  return renderToStaticMarkup(createElement(SearchSelectorControl,{value,connections,pending:false,error:'',notice:'',onSelect:vi.fn(),onFreshness:vi.fn(),onRefresh:vi.fn(),...extra}))
}
describe('compact native-style search picker',()=>{
  ...
  it('uses native model-picker metrics and keeps feedback out of normal layout',()=>{
    ...
    expect(selectorCss).toContain('--dsw-specific-menu');expect(selectorCss).toContain('border-radius:20px')
