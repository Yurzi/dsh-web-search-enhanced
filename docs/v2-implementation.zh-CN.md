...
- DSH：包要求 `>=0.1.7-rc.2`，源码以 0.1.7-rc.2 服务契约为基线。依赖声明与锁文件是构建依据，不修改宿主安装的依赖来让测试通过。
- 当前开发主线为 `main`，旧版代码线为 `legacy/v0.0.x`。包版本 `0.1.4` 与 settings schema 的 `version: 2` 不同。
...
3. 用户可用 `dsh plugin --profile web add /absolute/path/to/dsh-web-search-enhanced-0.1.4.tgz` 安装 Release 构建附件。
