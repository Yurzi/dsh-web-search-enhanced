...
本指南面向安装版插件。包版本为 **0.1.4**，配置格式的 `version` 为 **2**；两者不是同一版本号。快速开始见 [README](../README.md)，旧版用户先读[升级指南](migration.zh-CN.md)。
...
连接选择器的默认选项具备模型配对跟随机制：
- 用户在特定模型下通过连接选择器选择搜索连接后，该模型与搜索连接的配对关系将持久保存在状态缓存文件（`$DSH_HOME/cache/web-search-enhanced/model-connections.json`）。
- 后续为该模型初始化新会话时，将自动沿用上一次选择的配对连接（包括明确选择禁用搜索的 `null`）。
- 对于未曾配对过的新模型（或缓存中的配对连接已在配置中禁用/移除），则默认使用配置文件里的首选项（即 `defaultConnection`，未指定时按单可用连接回退）。
