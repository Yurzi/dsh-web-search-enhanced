...
const freshnessSchema = z.enum(['auto', 'fresh', 'realtime'])
export const setRequest = getRequest.extend({ connectionId: z.string().min(1).max(128).nullable().optional(), freshness: freshnessSchema.optional(), expectedRevision: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER) })
  .refine(value => value.connectionId !== undefined || value.freshness !== undefined, 'At least one selection change is required')
export const selectionSchema = z.object({ connectionId: z.string().nullable(), freshness: freshnessSchema.optional(), revision: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER) }).strict()
