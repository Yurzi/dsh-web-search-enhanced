...
try {
  ...
  const theme = await readFile(join(root, 'scripts/fixtures/dsh-theme.css'), 'utf8')
  await writeFile(join(temp,'index.html'), `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${theme}
*{box-sizing:border-box}body{margin:24px;font-family:Arial,sans-serif;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base)}.composer{position:absolute;bottom:60px;left:16px;right:16px;border:1px solid var(--dsw-alias-border-l2);border-radius:24px;background:var(--dsw-alias-bg-layer-1);padding:20px;container-type:inline-size}.placeholder{color:var(--dsw-alias-label-tertiary);height:68px}.toolbar{display:flex;align-items:center;gap:16px;justify-content:flex-end}.model{font-size:13px;color:var(--dsw-alias-label-secondary)}.model span{color:var(--dsw-alias-label-caption)}#result{display:none}
</style><body><script>document.body.toggleAttribute('data-ds-dark-theme',matchMedia('(prefers-color-scheme:dark)').matches)</script><pre id="result">waiting</pre><div class="composer"><div class="placeholder">发送消息或创建任务</div><div class="toolbar"><div id="picker"></div><div class="model">GPT-6 Astra <span>Medium⌄</span></div></div></div><script src="search-selector.iife.js"></script></body></html>`)
